...
export const postReviewScore = (eventId , userId , description , score , username) => async (dispatch) => {

  try {
    const result = await axios.post(`reviewScore/${eventId}` , {userId , description , score , username})
    return dispatch({
      type: "POST_REVIEW",
      payload: result.data
    })
